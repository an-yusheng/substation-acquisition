 ...
 */
package com.substation.acquisition.utils;
...
import static com.substation.acquisition.utils.FileUtil.SYS_TEM_DIR;
...
public class GenUtil {
    ...
    public static String download(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
        // 拼接的路径：/tmpsubstation-acquisition-gen-temp/，这个路径在Linux下需要root用户才有权限创建,非root用户会权限错误而失败，更改为： /tmp/substation-acquisition-gen-temp/
        // String tempPath =SYS_TEM_DIR + "substation-acquisition-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        String tempPath = SYS_TEM_DIR + "substation-acquisition-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        Map<String, Object> genMap = getGenMap(columns, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
        // 生成后端代码
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/front/" + templateName + ".ftl");
            String path = tempPath + "substation-acquisition-web" + File.separator;
